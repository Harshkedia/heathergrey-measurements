...
export const MeasurementInput = React.forwardRef<any, any>(
  (
    { selec, register, changeable, people, index, value, onChange, ...props },
    ref
  ) => {
    const [selected, setSelected] = useState(selec);

    useEffect(() => {
      if (!people || !ref) return;
      if (!people[index].measurements) {
      ...
      <div className="flex gap-4 items-center">
        <div className="w-[320px] relative">
          <Listbox value={selected} onChange={changeable && setSelected}>
            <Listbox.Button
              className={`relative w-full rounded-lg py-2 text-left focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm ${
